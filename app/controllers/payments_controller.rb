class PaymentsController < ApplicationController
  ...
  before_action :auther_user, except: [:pay_notify]
  before_action :acth_request, only: [:pay_return, :pay_notify]
  ...
  def generate_pay
    ...
    redirect_to payments_path(payment_no: payment_no)
  end
  ...
  def bulid_request_options payment
    # opts:
  ...
  def bulid_is_request_from_alipay? result_options
  ...
  def build_is_request_sign_vaild? result_options
  ...
  def build_rsa_sign(data)
    ...
    pri = OpenSSL::PKey::REA.new(File.read(private_key_path))
  ...
  # REA 验证
  def bulid_rsa_verify?(data, sign)
