class User < ApplicationRecord
  ...
  has_many :address, -> { where(address_type: Address::AddressType::User).order("id desc") }
  ...
  def validate_email_or_cellphone
    if[self.email, self.cellphone].all? { |attr| attr.nil? }
      ...
      if self.cellphone.nil?
        ...
        unless VerifyToken.availables.find_by(cellphone: self.cellphone, token: self.token)
